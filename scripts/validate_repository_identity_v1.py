#!/usr/bin/env python3
"""Fail closed when evidence binds to a non-canonical HeptaBao repository identity."""
...
DEPRECATED_REPOSITORY = "ProfAlex" + "QI/HeptaBao"
...
def main() -> int:
    try:
        deprecated = DEPRECATED_REPOSITORY.encode("utf-8")
        offenders: list[str] = []
        ...
        if offenders:
            fail("deprecated repository identity remains in: " + ", ".join(sorted(offenders)))
    ...
    print(
        ...
        f"repository={EXPECTED_REPOSITORY} tracked_files={len(tracked_paths())}"
    )
    return 0
