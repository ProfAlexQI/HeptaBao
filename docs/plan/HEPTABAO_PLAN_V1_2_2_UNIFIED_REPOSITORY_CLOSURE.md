...
PR #41 advanced from its prior transition head only to move the one-shot materializer from `ubuntu-slim` to `ubuntu-24.04`; the lifecycle payload remained the same and the materializer remained queued at revalidation.
