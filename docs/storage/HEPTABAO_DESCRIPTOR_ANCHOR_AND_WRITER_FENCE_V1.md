...
The root must already exist and be absolute. Its final component may not be a
symlink. Device/inode identity is observed before open, from the opened handle
and after open; all observations must agree. The `/proc/self/fd/<fd>` access
path must resolve to the same identity.
...
- nested `openat2(RESOLVE_BENEATH)` semantics;
