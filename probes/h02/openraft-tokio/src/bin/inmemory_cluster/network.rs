...
use openraft_memstore::{MemStateMachine, TypeConfig};
use tokio::sync::RwLock;

pub type MemRaft = Raft<TypeConfig, Arc<MemStateMachine>>;
...
#[derive(Default)]
struct RouterState {
    nodes: RwLock<BTreeMap<u64, MemRaft>>,
...
impl InMemoryRouter {
    pub async fn register(&self, id: u64, raft: MemRaft) {
    ...
    async fn target(&self, source: u64, target: u64) -> Result<MemRaft, RPCError<TypeConfig>> {
...
impl InMemoryNetworkFactory {
    pub fn new(source: u64, router: InMemoryRouter) -> Self {
        Self { source, router }
    }
}
...
impl InMemoryNetwork {
    async fn target(&self, rpc_kind: &str) -> Result<MemRaft, RPCError<TypeConfig>> {
        self.router.count(rpc_kind).await;
        self.router.target(self.source, self.target).await
    }
}

impl RaftNetworkV2<TypeConfig> for InMemoryNetwork {
