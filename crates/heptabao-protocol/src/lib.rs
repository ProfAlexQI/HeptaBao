...
pub const MAX_REQUEST_BUDGET_TICKS: u64 = 60_000_000_000;
...
pub fn parse_http_request(input: &[u8]) -> Result<ParsedHttpRequest, ProtocolError> {
    ...
    let head = std::str::from_utf8(&input[..head_end])
        .map_err(|_| ProtocolError::NonUtf8Head)?;
    if !head.is_ascii() {
    ...
    for line in lines {
        ...
        if headers.insert(canonical_name, value.to_owned()).is_some() {
            return Err(ProtocolError::DuplicateHeader);
        }
    }
    if !headers.contains_key("host") || headers.get("host").is_some_and(String::is_empty) {
...
impl RequestEnvelope {
    pub fn validate_at(&self, now: MonotonicTick) -> Result<Operation, ProtocolError> {
        ...
        let budget = self.deadline.0 - self.received_at.0;
        if budget > MAX_REQUEST_BUDGET_TICKS {
        ...
        if let Ok(request) = parsed {
            ...
            if let Ok(request_id) = request_id {
                let envelope = RequestEnvelope {
                    request_id,
                    request,
                    received_at: MonotonicTick(100),
                    deadline: MonotonicTick(110),
                };
                assert_eq!(envelope.validate_at(MonotonicTick(109)), Ok(Operation::SysHealth));
                assert_eq!(
                    envelope.validate_at(MonotonicTick(110)),
