...
pub fn verify_bound_assertion(
    ...
    let expected_digest = digest_provider.digest(&canonical_request)?;
    if !constant_time_equal(&expected_digest, &assertion.request_digest) {
        return Err(AuthbusError::RequestBindingMismatch);
    }
    let payload = assertion.unsigned_payload()?;
    if !signature_verifier.verify(&assertion.key_id, &payload, &assertion.signature)? {
...
impl ReplayCache for InMemoryReplayCache {
    fn check_and_record(
        ...
        if observed.contains_key(&key) {
            return Err(AuthbusError::ReplayDetected);
        }
        observed.insert(key, expires_at);
        Ok(())
    }
}
...
mod tests {
    ...
    fn assertion(request: &RequestBinding<'_>) -> AuthbusAssertion {
        let digest = TestDigest.digest(&request.canonical_bytes().unwrap_or_default());
